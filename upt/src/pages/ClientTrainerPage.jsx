...
import { getTrainers, getFilteredTrainers, } from '../services/trainerService';
import { getTrainerById, decrementDialogCount } from '../services/trainerService';
import {setTrainerToClient} from '../services/clientService'
...
function ClientsPage() {
    ...
    const fetchClients = async () => {
        try {
            ...
            for (let program of programs) {
                ...
                const clientsForProgram = await getFilteredTrainers(filterRequest);
    ...
    const handleSelectTrainer = async (trainerId) => {
        ...
        try {
            await setTrainerToClient(clientId, trainerId );
        } catch (error) {
            alert('Не удалось выбрать тренера.');
        }
    };
        ...
        <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ p: 2, backgroundColor: '#fff', position: 'sticky', top: 120, zIndex: 9, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Tabs value={tabValue} onChange={handleTabChange}>
                    <Tab label="Все тренера" />
                    <Tab label="Подходящие тренера" />
                </Tabs>
            </Box>
                                ...
                                >
                                    ...
                                    <Box sx={{ display: 'flex', gap: 1 }}>
                                        ...
                                        <Button
                                            variant="contained"
                                            color="secondary"
                                            onClick={() => handleSelectTrainer(client.id)}
                                        >
                                            Выбрать тренера
                                        </Button>
                                    </Box>
